...
class SynchDisk {
  public:
    ...
    void SyncRead(int sectorNumber, char *data);
    void SyncWrite(int sectorNumber, char *data);
  ...
  private:
    ...
    // ADD!!
    Semaphore *sem[NumSectors];
    Semaphore *mutex[NumSectors];
    int readers[NumSectors];
  
  public:
    int fileCount[NumSectors];
    // ADD!!
};
